...
FACILITY_CONFIG = {
    ...
    "support": {
        ...
        "icon": "home",
        "queries": [
            # Named services / shelters
            'nwr["name"~"one stop|sakhi|shakti sadan|women shelter|women support|child support|child care|child protection|bal griha|bal sadan|juvenile",i]',
            # OSM social-facility records explicitly intended for women/children
            'nwr["amenity"="social_facility"]["social_facility:for"~"women|child|children",i]',
            'nwr["social_facility"~"shelter|group_home|outreach|day_care",i]["social_facility:for"~"women|child|children",i]',
        ],
    },
}
...
def level_color(level):
    if "LOW" in level: return "#22c55e"
    if "HIGH" in level: return "#ef4444"
    return "#f59e0b"


def category_forecast_summary(full_df, district_df, label, col, years_ahead=3):
    result = forecast_category(district_df, col, years_ahead=years_ahead)
    if not result: return None
    future_df, mae, history_df = result
    years = sorted(district_df["year"].unique())
    recent_years = years[-3:] if len(years) >= 3 else years
    recent_value = float(district_df[district_df["year"].isin(recent_years)][col].mean())
    future_value = float(future_df["estimated_cases"].mean())
    future_level = level_from_percentile(future_value, category_reference_values(full_df, col))
    return {"label":label,"col":col,"recent_value":recent_value,"future_value":future_value,"future_level":future_level,"future_df":future_df,"mae":mae,"last_year":int(history_df["year"].max()),"recent_years":recent_years}
...
def get_nearby_facilities(lat, lon, radius_km, selected_types):
    ...
    def classify(tags):
        ...
        if office == "lawyer" or amenity == "legal_services":
            return "legal"
        # Women/child support can be mapped by service name OR by explicit
        # OSM social-facility tags. This keeps the map useful without inventing
        # facilities from the crime CSV.
        social_facility = tags.get("social_facility", "").lower()
        social_for = tags.get("social_facility:for", "").lower()
        support_terms = [
            "one stop", "sakhi", "shakti sadan", "women shelter",
            "women support", "child support", "child protection",
            "child care", "bal griha", "bal sadan", "juvenile"
        ]
        if (any(x in name for x in support_terms)
                or (social_facility and any(x in social_facility for x in ["shelter", "group_home", "outreach", "day_care"])
                    and any(x in social_for for x in ["women", "child", "children"]))):
            return "support"
        return None
...
def make_map(lat, lon, zoom, facilities, searched_name=None, pattern_level=None, radius_km=5):
    m = folium.Map(location=[lat, lon], zoom_start=zoom, tiles="OpenStreetMap", control_scale=True, prefer_canvas=True)
    Fullscreen(position="topright").add_to(m)
    pattern = pattern_level or "🟡 MODERATE"
    pcolor = level_color(pattern)
    folium.Circle([lat, lon], radius=radius_km*1000, color=pcolor, weight=3, fill=True, fill_color=pcolor, fill_opacity=0.08,
                  tooltip=f"Historical district pattern: {pattern}").add_to(m)
    folium.Marker([lat, lon], tooltip="📍 Selected / current location",
                  popup=f"<b>📍 Selected location</b><br>{searched_name or 'Current location'}<br><b>Historical pattern:</b> {pattern}",
                  icon=folium.Icon(color="blue", icon="user", prefix="fa")).add_to(m)
    legend_html = """
    <div style="position: fixed; bottom: 25px; left: 25px; z-index: 9999; background: rgba(17,28,45,.96); color: white; padding: 12px 14px; border: 1px solid #334155; border-radius: 10px; font-size: 13px; box-shadow: 0 2px 8px rgba(0,0,0,.35);">
      <b>🛡️ Historical Area Pattern</b><br>
      <span style="color:#22c55e">●</span> LOW &nbsp; <span style="color:#f59e0b">●</span> MODERATE &nbsp; <span style="color:#ef4444">●</span> HIGH
      <hr style="border-color:#475569; margin:7px 0;"><span>📍 Selected location</span><br><span>Markers = current mapped support</span>
    </div>
    """
    m.get_root().html.add_child(folium.Element(legend_html))
    for f in facilities:
        cfg=FACILITY_CONFIG.get(f["type"],{})
        popup=f"<b>{f['name']}</b><br>{f['type_label']}<br>Distance: {f['distance_km']} km<br>Address: {f['address']}<br>Phone: {f['phone']}"
        folium.Marker([f["lat"],f["lon"]], tooltip=f"{f['type_label']} • {f['name']} • {f['distance_km']} km", popup=popup,
                      icon=folium.Icon(color=cfg.get("color","blue"),icon=cfg.get("icon","info-sign"),prefix="fa")).add_to(m)
    return m
...
# ---------- Historical pattern + future estimates ----------
st.subheader("🛡️ Area-specific historical crime pattern")
overall=None; ddf=pd.DataFrame()
if matched_district:
    ddf=district_year_data(crime_df,matched_district); overall=overall_pattern(crime_df,ddf)
    if overall:
        st.markdown(f"""<div class="card"><div style="font-size:1.05rem;font-weight:700">📍 {selected_name}</div><div class="{level_class(overall["level"])}">{overall["level"]}</div><div class="small">Matched district: <b>{matched_district}</b> · Historical period: {overall["years_used"][0]}–{overall["years_used"][-1]}.<br>This is district-level historical context, not a street-level danger prediction.</div></div>""",unsafe_allow_html=True)
        results=[]
        for label,col in CRIME_CATEGORIES.items():
            r=category_pattern(crime_df,ddf,col)
            if r: results.append((label,col,r))
        st.markdown("### Crime-specific indicators")
        cols=st.columns(3)
        for i,(label,col,r) in enumerate(results):
            with cols[i%3]:
                st.markdown(f"""<div class="card"><b>{CATEGORY_EMOJI.get(label,"📌")} {label}</b><br><span class="{level_class(r["level"])}">{r["level"]}</span><br><span class="small">Recent-period average: {r["value"]:.1f} reported cases</span></div>""",unsafe_allow_html=True)
        trend_cols=[c for c in CRIME_CATEGORIES.values() if c in ddf.columns]
        if trend_cols:
            trend=ddf.groupby("year",as_index=False)[trend_cols].sum(); long_trend=trend.melt(id_vars="year",var_name="crime",value_name="reported_cases")
            name_map={v:k for k,v in CRIME_CATEGORIES.items()}; long_trend["crime"]=long_trend["crime"].map(name_map).fillna(long_trend["crime"])
            fig=px.line(long_trend,x="year",y="reported_cases",color="crime",markers=True,title=f"Historical reported women-related crime trends — {matched_district}"); fig.update_layout(template="plotly_dark",height=430,legend_title_text=""); st.plotly_chart(fig,use_container_width=True)
            latest_year=int(ddf["year"].max()); latest=ddf[ddf["year"]==latest_year][trend_cols].sum().sort_values(ascending=False); latest_df=latest.reset_index(); latest_df.columns=["crime","reported_cases"]; latest_df["crime"]=latest_df["crime"].map(name_map).fillna(latest_df["crime"])
            fig2=px.bar(latest_df,x="reported_cases",y="crime",orientation="h",title=f"Reported category comparison — {latest_year}"); fig2.update_layout(template="plotly_dark",height=430); st.plotly_chart(fig2,use_container_width=True)
    else: st.warning("The district was matched, but no usable historical crime records were found.")
else:
    st.info("The location was found on the live map, but no confident district match exists in the crime dataset. No crime level is invented.")

st.subheader("📈 Particular-crime past + future estimates")
if matched_district and not ddf.empty:
    forecast_rows=[]
    for label,col in CRIME_CATEGORIES.items():
        if col in ddf.columns and ddf[col].sum()>0:
            x=category_forecast_summary(crime_df,ddf,label,col,3)
            if x: forecast_rows.append(x)
    if forecast_rows:
        st.write(f"For **{selected_name}**, the dataset is district-level, so these estimates refer to **{matched_district} district** rather than an exact street.")
        table=pd.DataFrame([{"Crime type":x["label"],"Past avg. (latest up to 3 yrs)":round(x["recent_value"],1),"Next 3-year estimated avg.":round(x["future_value"],1),"Future relative level":x["future_level"]} for x in forecast_rows]); st.dataframe(table,use_container_width=True,hide_index=True)
        chosen=st.selectbox("Choose a particular crime to inspect past vs estimated future",[x["label"] for x in forecast_rows],key="forecast_category"); x=next(z for z in forecast_rows if z["label"]==chosen)
        hist=ddf.groupby("year",as_index=False)[x["col"]].sum(); hist.columns=["year","cases"]; hist["series"]="Historical reported cases"; fut=x["future_df"].rename(columns={"estimated_cases":"cases"}).copy(); fut["series"]="Model estimate"; plot_df=pd.concat([hist,fut],ignore_index=True)
        fig3=px.line(plot_df,x="year",y="cases",color="series",markers=True,title=f"{chosen} — {matched_district}: historical data + next 3-year estimate"); fig3.update_layout(template="plotly_dark",height=430); st.plotly_chart(fig3,use_container_width=True)
        st.markdown(f"""<div class="card"><b>{CATEGORY_EMOJI.get(chosen,"📌")} {chosen}</b><br>Past recent-period average: <b>{x["recent_value"]:.1f}</b><br>Next 3-year estimated average: <b>{x["future_value"]:.1f}</b><br>Estimated future relative level: <span class="{level_class(x["future_level"])}">{x["future_level"]}</span><br><span class="small">Time-aware validation MAE: {x["mae"]:.2f} cases</span></div>""",unsafe_allow_html=True)
        st.caption("Future values are statistical trend estimates from past district reports, not guaranteed events or individual-risk predictions.")
    else: st.info("Not enough historical observations are available for responsible category-level estimates in this district.")
else: st.info("Future estimates appear after a location is matched confidently to a district.")
...
if facilities:
    count_cols=st.columns(min(4,max(1,len(facility_options))))
    for i,key in enumerate(facility_options[:len(count_cols)]):
        count_cols[i].metric(FACILITY_CONFIG[key]["label"],sum(1 for f in facilities if f["type"]==key))

st.markdown(f"""<div class="card"><b>🗺️ Live support map for {selected_name}</b><br>Showing mapped support within approximately <b>{radius} km</b>. The colored circle represents the historical district pattern, not a street-level danger zone. Colored markers are current mapped support locations.</div>""",unsafe_allow_html=True)
